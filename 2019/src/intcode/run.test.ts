...
describe("run", () => {
  ...
  test("day 5", () => {
    ...
    expect(run(inputIsEqualTo8Pos, 8).output).toBe(1);
    expect(run(inputIsEqualTo8Pos, 2).output).toBe(0);

    const inputIsLessThan8Pos = [3, 9, 7, 9, 10, 9, 4, 9, 99, -1, 8];
    expect(run(inputIsLessThan8Pos, 2).output).toBe(1);
    expect(run(inputIsLessThan8Pos, 8).output).toBe(0);

    const inputIsEqualTo8Immediate = [3, 3, 1108, -1, 8, 3, 4, 3, 99];
    expect(run(inputIsEqualTo8Immediate, 8).output).toBe(1);
    expect(run(inputIsEqualTo8Immediate, 2).output).toBe(0);

    const inputIsNonZeroPos = [
      3, 12, 6, 12, 15, 1, 13, 14, 13, 4, 13, 99, -1, 0, 1, 9,
    ];
    expect(run(inputIsNonZeroPos, 0).output).toBe(0);
    expect(run(inputIsNonZeroPos, 8).output).toBe(1);

    const inputIsNonZeroImmediate = [
      3, 3, 1105, -1, 9, 1101, 0, 0, 12, 4, 12, 99, 1,
    ];
    expect(run(inputIsNonZeroImmediate, 0).output).toBe(0);
    expect(run(inputIsNonZeroImmediate, 8).output).toBe(1);
    ...
    expect(run(inputRelationTo8, 6).output).toBe(999);
    expect(run(inputRelationTo8, 8).output).toBe(1000);
    expect(run(inputRelationTo8, 10).output).toBe(1001);
  });
});
